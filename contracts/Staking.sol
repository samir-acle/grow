...
contract Staking is Pausable, ERC721Holder {
    ...
    /** @dev Check if token is staked.
      ...
      */
    function isTokenStaked(uint _tokenId) external view returns(bool isStaked) {
    ...
    /** @dev Get the number of tokens available for staking.
      ...
      */
    function getAvailableTokenCount() external view returns(uint count) {
    ...
    /** @dev Get the number of tokens being staking.
      ...
      */
    function getStakedTokenCount() external view returns(uint count) {
        return stakedTokens.length;
    }

    /** @dev Set the contract address that can stake and burn.
      * @param _controller address that acts as the controller
      */
    ...
    /** @dev Add token to the available stake pool.
      ...
      */
    function deposit(uint _tokenId) external whenNotPaused {
    ...
    /** @dev Stake token.
      ...
      */
    function stake(bytes32 _stakeId, uint _tokenId, address _staker)
        external 
        whenNotPaused 
        onlyController
    ...
    /** @dev Release staked token.
      ...
      */
    function releaseStake(bytes32 _stakeId, address _staker)
        external 
        whenNotPaused 
        onlyController
    ...
    /** @dev Burn staked token.
      ...
      * @param _staker The token owner.
      */
    function burnStake(bytes32 _stakeId, address _staker)
        external 
        whenNotPaused 
        onlyController
        onlyIfStaked(_stakeId)
        onlyOriginalStaker(_stakeId, _staker)
    {
        ...
        delete stakeIdToTokenId[_stakeId];
        growToken.burn(tokenToBurn, address(this));
    ...
    /** @dev Get address of staker.
      ...
      */
    function getStaker(bytes32 _stakeId)
        external
        view
        onlyIfStaked(_stakeId)
    ...
    function deleteAvailableTokenAtIndex(uint _index) private {
        ...
        availableTokens.length = availableTokens.length - 1;
    }
    ...
    function deleteStakedTokenAtIndex(uint _index) private {
        ...
        stakedTokens.length = stakedTokens.length - 1;
    }    
